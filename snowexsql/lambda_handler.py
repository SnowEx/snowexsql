"""
Lambda-specific helper that exposes SnowEx API endpoints.

It the core API functionality from api.py to expose the endpoints via lambda service.
This module adapts the existing snowexsql db helpers to accept credentials provided at
runtime via a temporary credentials.json file written from AWS Secrets Manager.

Conventions:
============
The automatic discovery engine of this modules assumes the following structure in api.py:

1. Class name MUST end with 'Measurements' (e.g., WeatherMeasurements)
2. Class MUST have a 'MODEL' attribute pointing to the SQLAlchemy model
...
See _get_measurement_classes() method in this module for implementation details.
"""
...
def _get_measurement_classes():
    ...
    LOG.info(f"Auto-discovered measurement classes: {discovered_classes}")

    if not measurement_classes:
        LOG.warning("No measurement classes found! Check naming conventions in api.py")
...
def validate_measurement_class(cls, class_name: str) -> bool:
    ...
    if not class_name.endswith("Measurements"):
        issues.append(f"Class name '{class_name}' should end with 'Measurements'")

    if not hasattr(cls, "MODEL"):
        issues.append(f"Class '{class_name}' missing required MODEL attribute")

    if not callable(cls):
        issues.append(f"'{class_name}' is not callable (not a class?)")

    # Check if it has expected BaseDataset methods
    expected_methods = ["from_filter", "from_unique_entries"]
    for method in expected_methods:
        if not hasattr(cls, method):
            issues.append(f"Class '{class_name}' missing expected method '{method}'")

    if issues:
        LOG.warning(f"Class '{class_name}' validation issues: {issues}")
        return False
...
def _handle_class_action(
    class_name: str, method_name: str, event: dict, tmp_cred_path: str
):
        ...
        if method_name == "from_filter":
            ...
            try:
                ...
                return _create_response(
                    action, serialize_for_json(records), count=len(records)
                )
            ...
            action = f"{class_name}.{method_name}"
            return _create_response(action, serialize_for_json(result), columns=columns)
            ...
            if hasattr(api_instance, method_name):
                ...
                raise ValueError(f"Property {method_name} not found on {class_name}")

        else:
            raise ValueError(f"Unsupported method: {method_name}")

    except Exception as e:
        return _create_error_response(f"{class_name}.{method_name}", e)
...
def _write_temp_credentials(creds: Dict[str, Any], dest: Path):
    ...
    cred_entry = {
        "username": (
            creds.get("username") or creds.get("user") or creds.get("db_user")
        ),
        "password": creds.get("password"),
        "address": creds.get("host") or creds.get("address"),
        "db_name": (
            creds.get("dbname") or creds.get("database") or creds.get("db_name")
        ),
    }
...
def handle_event_with_secret(
    event: Dict[str, Any], secret_dict: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if "." in action:
            ...
            result = _handle_class_action(
                class_name, method_name, event, str(tmp_creds)
            )
            session.close()
            return result
        ...
        if "session" in locals():
            session.close()
        return {"error": str(e), "action": event.get("action", "unknown")}


def _get_secret(secret_name: str, region_name: str = None) -> Dict[str, Any]:
    """
